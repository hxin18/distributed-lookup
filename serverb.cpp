...
int main(){
    ...
    serveratA_addr.sin_port = BPORT;
    
    bind(sockatA, (struct sockaddr*)&serveratA_addr, sizeof(serveratA_addr));
    cout << "The Server B is up and running using UDP on port " << BPORT << endl;
    ...
    while (1){
        ...
        if(strlen > 0){
            ...
            cout<<"The Server B received input <"<<query<<"> and operation <"<<fct<<">"<<endl;
            vector<string> res;
            vector<string> resSimM ;
            vector<string> resSimW ;
            if(!strcmp(fct,"prefix")){
                res = database->retrivePrefix(query);
                cout<<"The Server B has found < "<<res.size()<<" > matches"<<endl;
            }
                ...
                cout<<"The Server B has found < "<<res.size()<<" > matches and < "<<resSimW.size()<<" > similar words"<<endl;
            }
            ...
            sprintf(bufSend, "%ld", res.size());//int to ascii
